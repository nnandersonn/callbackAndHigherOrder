...
// CODE HERE
const postTaxPrices = prices.map(price => price * (1 + .08))
...
const totalPopulation = populations.reduce(function(arr, curr) {
  return arr + curr
} )
...
// CODE HERE
const orderTotals = orders.map((element, index) => {
  //console.log(element.price, element.tax)
...
// CODE HERE
const bobsTotal = purchases.filter(element => element.owner == `Bob`).map((element, index) => element.price).reduce((acc, cv) => acc + cv)
